...
- A primary user action (create/add/start) plus an empty/first-run state, but no specification that the action's **entry point is reachable from that empty state** — an affordance that only appears once data exists strands the first-run user (the create-from-empty miss)
- "Handle concurrent requests" (but no concurrency strategy)
- "Deal with large files" (but no size limits or chunking strategy)

**Why it matters:** Edge cases discovered during implementation cause scope creep. For each primary action, confirm the plan names a reachable entry point in *every* state the user would invoke it from — especially the empty/first-run state.
