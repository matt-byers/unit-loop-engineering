...
**Checklist — env-var-controlled debug/test switches:**
When reviewing any code path gated on an environment variable (look for `os.environ.get`, `process.env`, env var reads inline):
- [ ] Is the flag read via the **typed settings/config object** (e.g. Pydantic `Settings`, `dotenv` config) rather than raw `os.environ.get`? Raw reads are invisible to config audits and documentation.
- [ ] Is there an explicit **production exclusion** — a check that prevents activation when `app_env` / `NODE_ENV` is `production` or `staging`, regardless of how the flag is set? A single-flag gate is exploitable if the var is accidentally set in a prod deployment.
- If either check fails → flag `ARCHITECTURAL_CONCERN P1 BLOCKING`.

### 9. File Discovery Coverage

When a new file is added under a directory tree that a **runner, loader, or scanner** searches (eval scenarios, fixtures, migrations, test data, plugin directories):
- [ ] Identify the scan code (glob pattern, `os.walk`, import path, etc.).
- [ ] Verify the pattern **covers the new file's full path** — a flat `glob("*.yaml")` does not discover `subdir/*.yaml`; it requires `rglob("*.yaml")` or `**/*.yaml`.
- [ ] If the new file is unreachable by the existing scan: flag `CONFIGURATION_VERIFICATION P2 HIGH` and specify which scan needs updating.
