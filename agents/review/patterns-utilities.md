...
description: "Use this agent to review code for pattern violations and missed utility usage. This agent identifies reimplemented existing code, missing pattern usage, and violations of established codebase conventions. Should be invoked during /multi-review.\n\nExamples:\n- <example>\n  Context: Reviewing new feature code.\n  user: \"Check if I'm following existing patterns\"\n  assistant: \"I'll analyze the code against existing utilities and patterns in the codebase.\"\n  <commentary>\n  Use patterns-utilities agent to find duplicated logic and pattern violations.\n  </commentary>\n</example>"
model: inherit
---
...
- **MAKE_SELF_DOCUMENTING** (~6% frequency) - Tier 2 (Cleanliness)
...
### 7. Not Self-Documenting (MAKE_SELF_DOCUMENTING)

Magic values, opaque regexes, and non-obvious thresholds should be made readable by **naming**, not by adding a comment. This project does not use code comments.
...
**Should be** — extract named constants / helpers so the code reads itself:
```typescript
const UUID_V4_PREFIX = /^[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}/;

const MAX_RETRIES = 3;
const RETRY_BUDGET_MS = 5000;
if (retryCount > MAX_RETRIES && elapsed > RETRY_BUDGET_MS) { ... }
```
