...
### No Comments
**Rule:** Do not add comments. Code should be self-documenting — clear names, small functions, named constants. This includes doc comments (`///`) and section dividers (`// MARK:`).

**Why:** Comments drift out of sync with the code and read as wasted tokens; a coding agent over-produces them. A good name carries the same context permanently. See `SUPERFLUOUS_COMMENT`.

```typescript
// BAD - a comment papering over an unclear value
const timeout = 120000; // request timeout

// GOOD - the name carries the meaning, no comment needed
const AGENT_REQUEST_TIMEOUT_MS = 120_000;
```

### Make Unusual Patterns Self-Documenting
**Rule:** Unusual patterns, magic values, and workarounds are made readable by naming/restructuring, not by an explanation comment.

**Why:** A named constant or extracted helper survives refactors; a comment beside it drifts.

Cases to make self-documenting (not comment):
- Hardcoded values with non-obvious rationale → named constant
- Opaque regexes → named constant
- Workarounds for library bugs → named helper, plus a linked issue if truly non-obvious
- The rare gotcha no name can capture → at most one short line
