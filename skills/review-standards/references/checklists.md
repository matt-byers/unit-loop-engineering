...
- [ ] A side effect that must fire on **every path to a state** (haptic, analytics, log, telemetry) is owned by the **shared choke point** those paths route through, not wired per-action — enumerate all triggering actions and confirm none can bypass it
...
- [ ] **No doc comments (`///`) or `// MARK:` dividers**
- [ ] Magic numbers extracted to **named constants** (not explained with comments)
- [ ] Unusual patterns/workarounds made readable by **naming/restructuring**
